...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-79f138dd";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-79f138dd">'+'<span class="tile-face-79f138dd tile-front-79f138dd"></span>'+'<span class="tile-face-79f138dd tile-back-79f138dd"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-79f138dd")){return;}
var back=tile.querySelector(".tile-back-79f138dd");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-79f138dd");if(isMine){tile.classList.add("boom-79f138dd");}
if(dim){tile.classList.add("dimmed-79f138dd");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-79f138dd")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-79f138dd");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-79f138dd")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-79f138dd");void panelEl.offsetWidth;panelEl.classList.add("jolt-79f138dd");}
endRound();}
